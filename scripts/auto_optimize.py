...
Runs benchmark iterations in isolated nix develop environment.
...
def get_benchmark_result():
    ...
    cmd = ["nix", "develop", "--command", "python3", "scripts/benchmark.py"]
...
def prompt_optimization_agent(iteration: int, current_latency: float):
    """
    Prompt the AI agent for an optimization diff.
    Supports calling agy CLI or prompt instructions.
    """
    prompt = (
        f"[Iteration {iteration}] Current Best Latency: {current_latency:.3f} ms.\n"
...
def main():
    ...
    print("⚡ Establishing initial performance baseline in Nix environment...")
    baseline = get_benchmark_result()
    
    if not baseline["passed"]:
        print(f"❌ Initial baseline failed test suite! Reason: {baseline.get('reason')}")
        sys.exit(1)
    ...
    if args.dry_run:
        print("\n[Dry Run Complete] Baseline successfully measured.")
        sys.exit(0)

    for i in range(1, args.iterations + 1):
        prompt_optimization_agent(i, best_latency)
        
        # Give agent or user time to apply change if interactive, or trigger agy CLI
        input("\nPress Enter after code change is applied to run benchmark (or Ctrl+C to stop)... ")
        
        print("\nRunning verification benchmark in Nix shell...")
        res = get_benchmark_result()
        
        if res["passed"] and res["latency_ms"] < best_latency:
            speedup = best_latency / res["latency_ms"]
            diff_ms = best_latency - res["latency_ms"]
            print(f"\n🚀 WINNING OPTIMIZATION DETECTED!")
            print(f"   Speedup : {speedup:.2f}x")
            print(f"   Latency : {best_latency:.3f} ms -> {res['latency_ms']:.3f} ms (saved {diff_ms:.3f} ms)")
            
            best_latency = res["latency_ms"]
            subprocess.run(["git", "commit", "-am", f"perf: speedup {speedup:.2f}x ({res['latency_ms']:.3f}ms)"])
        else:
            ...
            print("   Reverting working tree to last clean commit...")
